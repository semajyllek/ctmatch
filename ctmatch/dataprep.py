...
class DataPrep:
    ...
    def load_data(self) -> Dataset:
        ...
        if not self.model_config.use_trainer:
            self.ct_dataset = self.ct_dataset.remove_columns(['doc', 'topic', 'labels'])  # removing labels for next-token prediction...
    ...
    def create_doc_embeddings(self, split='train'):
        ...
        doc_embeddings = norm(np.array(doc_embeddings))
    ...
    def get_embedding_similarity(self, topic, document):
        ...
        topic_output = self.lm_model(**topic_input, output_hidden_states=True)
        doc_output = self.lm_model(**doc_input, output_hidden_states=True)
