...
## 14. Synthetic narrative (patient, trial) pairs — attacking the data wall (2026-07)

The §13h program conclusion is that beating 2022 SOTA collapses to a single question — *can monoT5-3B be domain-adapted where six attempts failed?* The six `monoT5_CT` failures (§7e) all catastrophically forgot; the diagnosed root cause is **data**, not method: the only in-domain relevance data is KZ (~1.1k positives that are **10-word one-liners**), while TREC patients are **~150-word narratives**, so KZ→TREC does not transfer and the fine-tune overwrites the strong base rather than adapting it. `finetune_monot5_ct_v7.ipynb` attacks this from the **regularization** side (KL-to-base anchor); this section adds the complementary **data** side — manufacture the right-distribution supervision KZ never was.

### 14.0 What the data actually is, and what it is for (read this first)

Three things are easy to lose in the pipeline detail:

- **Which side is synthetic: the query, not the document.** Every training row is `(patient_note, trial_fields, label)`. The **patient note is a synthetic *topic/query*** (Claude-generated — the analog of a TREC topic). The **trial is a *real document*** sampled straight from the 374k-trial corpus (`load_corpus`), with its actual title/condition/eligibility — **nothing about the document is fabricated**. The **label is constructed** (gold-by-construction). Cross-patient negatives are the same shape (`synthetic patient A`, `real trial B` of a different condition, `false`). So the model never learns a fake document distribution — it learns to read *real* trial eligibility against *narrative* patients, which is exactly the TREC task. Fabricating queries fills the gap that actually exists; fabricating documents would invent one that doesn't.

- **What it targets: TREC22, not KZ.** KZ is **not** a goal — it is auxiliary real training signal. TREC21 is the **develop/tuning** mirror; **TREC22 is the held-out SOTA target**. The synth is distribution-matched to *TREC-narrative style* (21 and 22 share the format), so it lifts both, but the design intent is 22. It makes **no attempt** to improve KZ.

- **How it reconciles the KZ↔TREC gap with the TREC22 goal — the synth *is* the bridge.** The gap is entirely on the **query side** (KZ ~10-word one-liners vs TREC ~150-word narratives; the *documents* are the same ClinicalTrials.gov trials). That gap is *why* fine-tuning on KZ alone collapses on TREC — you train the wrong query distribution and destroy narrative reading (measured: KZ-only at the winning operating point → 0.40, canary → 3%, §15c). The synth manufactures the **missing narrative distribution**, deliberately style-seeded from real TREC21 topics (the `trec_narrative` style is 60% of generations, few-shot from recorded donor topics), so the synthetic queries look like TREC patients, **not** KZ one-liners. Training then becomes **KZ + synth**: KZ supplies real eligibility-labeled signal, synth supplies the narrative distribution → a model with *both* eligibility discrimination and narrative reading, which is the TREC22 task. It bridges *from* the KZ-only training data *to* the TREC test distribution — it is the narrative data KZ never was. Contamination guards keep this legitimate: trials judged in TREC21/22/23 are excluded from the synth pool (no doc leakage), the TREC21 style-donor topics are recorded and excluded from v7 dev+canary (no query-side inflation), and TREC22 topics never touch generation at all. **Honest residual:** the synth is an *approximation* of the TREC distribution — the difficulty gate proved v1 was ~90% *easier* than real TREC positives (the generator lexically echoed the eligibility text), which is why the huge develop gain (§15) transferred to TREC22 as a smaller +0.06 (§16) and why `v3_implicit` (harder synth) exists.

### 14a. `gen_synthetic_pairs.ipynb` — gold-by-construction, verified for fidelity

Inverts the annotation problem (silver labeling of real pairs → sample the target label, then generate the patient to realize it). Design decisions (all deliberate):

- **Grain = whole-trial graded relevance**, TREC semantics: `rel=2` Eligible (has condition, meets all inclusions, trips no exclusion), `rel=1` Excluded (*has the condition — on-topic — but* fails an inclusion or trips an exclusion), `rel=0` Not Relevant (different condition).
- **Generate only rel≥1; mine rel=0.** rel=0 is trivially easy, weak-gradient, and abundant from the consumers' existing dense hard-negative mining. Every generation dollar goes to the scarce, load-bearing classes — **rel=1 first** (the on-topic-but-ineligible narrative is exactly the eligibility-discrimination signal 2022 rewards and exactly what KZ's one-liners cannot provide), rel=2 second. Default mix 60/40.
- **Trials sampled from the corpus** (`load_corpus`), not the API: representation-consistent with frozen `R = elig_first-L512`, title/condition/summary/eligibility free, and the home trial has a dense embedding so the judge's negative-mining works against it. inc/exc split with ctproc's `process_eligibility_naive`.
- **Compose the label deterministically, don't ask for a holistic 3-way judgment** (noisiest exactly on the 2-vs-1 boundary we care about). Sample decisive per-criterion targets + an on-topic regime; `rel=1` ⟺ exactly one disqualifier. Verification (independent Haiku) is then two cheap checks — a **topicality** yes/no and the archived **per-criterion** blind labeling — and the pair is kept only when topicality agrees *and* every criterion's blind label matches its target.
- **Honest framing:** this is LLM-distilled supervision; blind-verify is a **generation-fidelity filter** ("did the note realize the spec?"), *not* independent ground truth. A **pilot cell** (≈60 patients) prints the per-class agreement matrix *before* the full run — rel=1 is the MVP and the most fragile (a drifted "excluded" patient can collapse toward off-topic rel=0), so it is inspected first.
- **Contamination guard:** trials judged in **TREC21 + TREC22 + TREC23** are excluded from sampling. TREC21 is excluded *even though it is develop data* because it is the go/no-go **dev** arbiter — leaving its trials in the pool would let a dev-NDCG lift come from trial-text familiarity rather than genuine distribution transfer, and transfer is the whole claim. Both output files resumable; Claude for data-gen only (no closed model on the inference path).

### 14b. Smart-synthesis surface — config, difficulty diagnostic, and the downstream A/B

The generator is built to *optimize* the data, not just produce it — "best bang per example, hard without catastrophic forgetting" (user directive):

- **Config + registries.** A frozen `SynthConfig` + `MODELS` / `GEN_PROMPTS` / `TOPIC_PROMPTS` / `VERIFY_PROMPTS` registries selected by key; everything (models, prompts, sampling, curation thresholds) reads through `cfg`. `cfg.tag()` (e.g. `sonnet.v1__haiku.v1`) stamps every output file *and* every row's `gen_tag`, so prompt/model variants A/B without collision and any downstream result attributes to the exact config that made the data. Swapping a prompt or model = one key. (`v2_strict` ships as a real alternate generator prompt.)
- **Difficulty is a DIAGNOSTIC, not a keep-filter.** An optional GPU cell scores every verified pair with **base monoT5-MED** (`castorini/monot5-3b-med-msmarco` — the exact model v7 adapts) and histograms the true/false margin by gold label. Curation policy: **cap** trivially-correct positives (`margin ≥ easy_margin`, low gradient; keep `easy_keep_frac` as anti-forgetting anchors), **keep** the mid band (learnable-but-nontrivial), and **route** base-strongly-disagrees positives (`margin ≤ disagree_margin`) to a **QA queue file, not the training set**. The reasoning (advisor-vetted): for a *positive*, monoT5 flatly disagreeing while the independent verifier passed it is disproportionately a **fidelity failure**, and auto-including it injects label noise into the high-gradient region — which is the *forgetting* direction. So the notebook deliberately does **not** maximize "hard"; the v7 **canary** arbitrates the hard-vs-forgetting tradeoff. (Base-margin difficulty as a *keep-filter* is correct only for the mined **negatives**, which carry real qrel labels — that logic already lives in v7's KZ hard-negative mining.) The same cell also **calibrates synth hardness against real TREC21**: it scores a sample of real TREC21 judged pairs (topic note + judged trial, a calibration reference only — never train/dev) with the same base model and overlays the margin distributions, reporting *% of synth positives easier than the median real positive* (~50% = matched hardness; ≫50% = the generator's evidence is too explicit → weak gradient + transfer risk → tighten the prompt or upgrade the generator). This is the **pre-spend "is the generator good enough?" gate** — a fidelity-high but too-easy generator is the specific way synthetic→real transfer fails, and it is invisible to blind-verify. Runs on the in-memory pilot (`SYNTH_SOURCE='pilot'`) *before* the full generation, or on the saved file (`'file'`) after, which additionally curates.
- **Whole-trial relevance gate (fixes a real labeling weakness found by reading the QA queue).** The per-criterion verifier confirms the note realizes the *sampled* criteria, but `rel` is a *whole-trial* claim. Inspecting the first full run's base-disagrees QA queue (237 pairs) revealed the dominant failure: when a spec samples one generic criterion from a multi-criterion trial and the trial's core context is narrow, a patient can satisfy that one criterion while being the **wrong kind of patient** for the trial — mislabeled rel≥1 that is really rel=0 (a knee patient labeled Eligible for a *temporomandibular*-joint trial; a lymphoma-no-COVID patient for a COVID T-cell-therapy trial; a healthy 55yo woman for a "healthy male 20–45" PK study). base monoT5 was *correct* to reject these, validating the QA-routing — but the same weakness (weak topicality guard on a broad `conditions[0]`; 72% of the QA queue were single-criterion specs) produces milder mislabels across the whole set. Fix: an **independent whole-trial relevance check** (`cfg.verify_model`), run on the existing pairs file — **one call per pair, no regeneration** — that drops mismatches and reports the drop rate by rel. Resumable (`cfg.relevance_path()`, keyed by the relevance prompt); the difficulty/curation cell consumes it, cleaning the set before curation. **⚠ Gate-prompt calibration is load-bearing and non-obvious.** The first framing (`v1`, a *candidacy* check — "is this patient a candidate?") **catastrophically over-dropped rel=1: 91% of rel=1 vs 22% of rel=2** on the full run. Reason: a rel=1 patient is *ineligible by construction* (the spec flips a criterion to fail an inclusion / trip an exclusion), so a candidacy judge correctly rejects nearly every *valid* rel=1 — it conflated "ineligible-but-relevant" (keep) with "irrelevant" (drop). The reframe (`v2_topical` — *pure topical relevance*, "ineligibility ≠ irrelevance") **also failed: still 84% of rel=1 dropped vs 17% of rel=2**, and the same clearly-valid pairs (a contraception patient ↔ a contraception trial) kept getting culled. The cause is **structural, not prompt-fixable**: a rel=1 patient is *ineligible by construction*, so any relevance judge scores its salient disqualifying feature down. **Conclusion — the whole-trial relevance gate is abandoned** (`RUN_RELEVANCE_GATE`/`APPLY_RELEVANCE_GATE` default `False`; kept as a documented negative result). It is also *unnecessary for the primary consumer*: monoT5 v7 is **pointwise** (`rel≥1 → 'true'`), so a valid rel=1 is a *correct* positive and dropping it removes good signal, while the genuinely harmful mislabels are either already caught by the difficulty curation's `base_disagrees` bucket or are low-gradient (monoT5 already scores them plausibly) — net-negative to gate. Residual mislabel noise is left for the **v7 A/B + canary** to arbitrate. **Meta-lesson:** across this build the risk consistently lived in the *scoring/filtering* layer (the taxonomy-collapse fix, then two failed relevance-gate rounds) while the generation itself was solid — a standing caution to stop over-filtering and let the downstream number decide. If mislabels must be addressed, do it *at generation* (Fix B: always include the trial's core inclusions in the spec, not one cherry-picked criterion), not with a post-hoc gate.
- **The "bang for buck" meter is the downstream A/B, not a dashboard.** v7's **TREC21 dev-NDCG** (value) and **canary-gap retention** (no-forgetting) already are the two metrics; a variant is evaluated by running v7 twice — `SYNTH_PAIRS_PATH=''` (KZ-only) vs the curated file — and diffing a thin `synth_ab_results.jsonl` row (keyed by `gen_tag`, logged by a v7 cell). Model-free triage (distinct-conditions count, exact-dup rate) runs pre-spend to catch mode collapse before paying Claude. Diversity/embedding-distribution proxies were deliberately *not* built — this project's history (§11a CoT dev-null, §11c judge-FT ensemble-null, §11d listwise flat) is a graveyard of plausible proxies that didn't predict the downstream number, so the canary is the arbiter.
- **Cost honesty.** Post-hoc difficulty curation raises *value-per-example at train time* but does **not** reduce generation spend — Claude is paid before monoT5 scores. The only dollar lever is *pre-generation spec targeting* (choosing trials/criteria where the base is weak, scorable before generating the patient); recorded, not built.

### 14c. Wiring — one JSONL, two consumers

The generator emits one self-contained `synthetic_pairs.jsonl` (each row carries the inline corpus `fields` dict, so no consumer needs a corpus join); each consumer collapses the graded label as it needs:

- **`finetune_monot5_ct_v7.ipynb` (PRIMARY — the live §13h 2022-beat lever).** Optional cell after the KZ example-build: appends `rel≥1 → 'true'` synthetic positives (the 2-vs-1 grade is invisible to a pointwise reader). Because synthetic patients have no home-trial negatives, it adds **cross-patient negatives** (another synthetic patient's trial with a *different condition*) so the register is narrative on both sides and the model cannot shortcut "narrative ⇒ true." No-op if the file is absent.
- **`finetune_judge_lora.ipynb` (secondary — §11c says this pointwise-eligibility axis is near-saturated, so lower EV).** Optional cell after `build_groups`: each verified synthetic patient becomes one **listwise group** — home trial as the graded positive (its text injected into `id2txt`) + dense-mined real-corpus hard negatives (reusing the exact machinery `build_groups` uses for real topics). Appended to **train only**; dev stays real held-out TREC21.

**Anti-gaming (both):** go/no-go is read only on real **TREC21 dev**, TREC22 spent **once** — never read success off synthetic data.

**Status:** built, run, and (below, §15) **validated with a decisive result.** The generation pipeline produced **3,942 curated narrative positives** (`sonnet.v1__haiku.v1`; 2,424 rel=1 + 1,832 rel=2, topicality 95%, 1,062 conditions, p50 194 words, 0 dups), after several *filtering*-layer corrections that are themselves instructive: (a) the per-criterion verifier freely says "excluded" for an inclusion a patient fails, so **verify on the collapsed satisfies/disqualifies/NEI decision, not the raw 4-way label** (recovered ~1,460 pairs; rel=1 yield 28%→69%); (b) a **whole-trial relevance gate was abandoned** — both a "candidacy" and a "topical" prompt over-dropped rel=1 by 84–91%, because a rel=1 patient is *ineligible by construction* so any relevance judge culls it; for the pointwise monoT5 consumer a valid rel=1 is a *correct* positive, so the gate is net-negative (kept as a documented negative result, default off). **Meta-lesson across the whole build: the generation was solid throughout; every problem lived in the scoring/filtering layer** — a standing caution to prefer letting the downstream number decide over adding another filter. Style-donor TREC21 topics are recorded to `synth_style_donors.json` and excluded from v7's dev + canary (closes the query-side arbiter confound). See §15 for the v7 A/B result.

---

## 15. The result — synthetic data breaks the monoT5 adaptation wall (TREC21 develop, 2026-08)

§13h reduced the whole 2022-beat question to one thing: *can monoT5-3B be domain-adapted where six prior attempts catastrophically forgot?* §14 built the right-distribution data (KZ is 10-word one-liners; TREC is ~150-word narratives). This section is the payoff: feeding the synth into `finetune_monot5_ct_v7.ipynb` (KL-to-base anchor + gentle LoRA r8/α8 q-v, LR 1e-5) **breaks the wall**, and the effect is **decisively attributable to the synthetic data.**

**Table 15.1 — TREC21 judged-pool NDCG@10 (develop; `castorini/monot5-3b-med-msmarco` base = 0.449; 6 prior fine-tunes 0.14–0.20; h2oloo `monoT5_CT` = 0.7118):**

| λ_KL | steps | data | dev NDCG (retention-sel / *canary*) | **dev NDCG (final/trained)** | canary at end |
|---|---|---|---|---|---|
| 3 | 500 | KZ-only | 0.4527 | — | collapsed after step 50 |
| 3 | 500 | KZ+synth | 0.4793 | — | **held (rose to 103%)** |
| 1 | 500 | KZ+synth | 0.4543 | **0.5288** | held (~95%) |
| 1 | 1000 | KZ+synth | 0.4544 | **0.5954** | held (89%) |
| 1 | **1500** | KZ+synth | — | **0.6132** | held (~100%; declines to 80% mid-run) |
| 1 | 1000 | **KZ-only** | 0.4539 | **0.4002** *(< base)* | **collapsed → 3%** |

At λ_KL=1 the dev-vs-step curve (KZ+synth, `EVAL_STEPS` periodic-checkpoint eval) climbs monotonically and **decelerating** — 250:0.4829 · 500:0.5284 · 750:0.5641 · 1000:0.5923 · 1250:0.6072 · **1500:0.6132** — i.e. it clears the **0.60 GO bar** by step 1250 and is near its develop ceiling (~0.615) by 1500. The recipe was frozen at **step 1250** (dev 0.6072) rather than dev-max 1500: the canary falls 86%→80% over 1000→1500 while dev rises only +0.021, so the last steps are partly TREC21-develop-specific and likely worse for TREC22 transfer. (Reproducibility: the two λ_KL=1/500 rows in `synth_ab_results.jsonl` reproduce to 0.4542/0.4543 — the runs are stable. Caveat on that log: its `trec21_dev_ndcg` field means *best-canary* in early rows but *develop-best* in the 1500 row — read `trec21_dev_ndcg_final`/`dev_curve` for cross-row comparison.)

Three findings, in the order they were discovered:

### 15a. The KL anchor solves catastrophic forgetting (first non-collapse in seven attempts)
At λ_KL=3, KZ+synth adapts to **0.4793 with canary retention 103%** — no collapse, where §7e's six attempts all cratered to 0.14–0.20. The explicit `KL(base‖ft)` penalty on the first-token distribution is the axis the prior six never attacked, and it works. (KZ-only at λ_KL=3 stays afloat too, at 0.4527 — the strong anchor prevents collapse but also throttles learning to ≈base.)

### 15b. The "~0.48 plateau" was a checkpoint-selection artifact, not a recipe ceiling
The notebook checkpoint-selected on the **canary** (10-topic pos−neg gap). That is a *bad quality proxy*: adding a save-final + dual-eval showed the retention-selected checkpoint (0.4543) was massively under-selling the **trained** checkpoint. At λ_KL=1, the model keeps learning (task loss falling) while the canary drifts down mildly — and the **final** adapter scores **0.5288 (500 steps) → 0.5954 (1000 steps)**, a clean monotonic climb (0.454→0.529→0.595) that was entirely hidden by canary-based selection. **Lesson: do not checkpoint-select on the canary; select on the TREC21 develop set** (TREC22 stays untouched). A 5%-canary dip corresponded to a **+0.075** dev gain — the canary over-penalizes the mild forgetting that accompanies real learning.

### 15c. Attribution — the synthetic data is the load-bearing ingredient (Δ = +0.195)
At the operating point that reaches the GO bar (λ_KL=1, 1000 steps, final checkpoint): **KZ+synth = 0.5954** (canary held 89%) vs **KZ-only = 0.4002** (canary **collapsed 1.94→0.06**, dev *below base*). The mechanism is now explicit and proven at the point that matters: reaching 0.60 requires the **weak** anchor (λ_KL=1) so the model can learn enough — but at λ_KL=1 the 6.6k KZ one-liners overfit and **destroy narrative competence** (the canary/TREC are narrative), collapsing dev below base; the ~4k **narrative** synth positives are the only thing that lets the weak anchor run without collapsing. This is the §13h "KZ→TREC doesn't transfer" diagnosis *and its synthetic-data fix*, demonstrated end to end.

**Honest residual:** KZ+synth carried 3.4× more examples (22,386 vs 6,618), so a pure data-*quantity* effect is not 100% excluded. What tips it firmly toward *distribution* is the **specificity** — narrative synth holds the **narrative** canary while one-liners collapse it; a quantity story would require more one-liners to help (they wouldn't). The one untested refinement is whether *generic* clinical narrative would work as well as spec-realized synth.

**Standing & caveats.** Develop **peaked at 0.6132** (step 1500) and cleared the 0.60 GO bar by step 1250; recipe frozen at **step 1250 (0.6072)** for transfer. Synth is decisively attributed (Δ=+0.195 vs KZ-only, which *collapses* below base at the same operating point). This is **TREC21 develop, single run, n=72** — the monotonic curve makes the *direction* rock-solid, but the absolute number is not final, and it is the TREC21 *judged* pool (easier than a retrieval pool). vs h2oloo's `monoT5_CT` 0.7118 on the same metric, the develop gap is ~+0.10 (two turns of analysis ago it was mistakenly thought to be a hard ~0.48 ceiling). **The real test — §16 — is the TREC22 one-shot** (`eval_trec22_oneshot.ipynb`, built): frozen step-1250 adapter reranks the **TREC22 NQS pool** in v7's native vdoc format, graded NDCG@10, vs the **fair same-pool bar 0.5750** (our ensemble) and the full-corpus SOTA reference **0.6125** (h2oloo), with a same-format zero-shot control for the clean adaptation delta. Discipline: one checkpoint, one eval, no peeking at the other five. Optional hardening after: seed-repeats; a generic-narrative control for the quantity residual; the reserved *implicit-evidence / stronger-generator* synth lever if more headroom is needed (the difficulty gate flagged the current synth as ~90% easier than real TREC positives).

---

## 16. The TREC22 one-shot — method validated, not a standalone SOTA beat (2026-08)

Everything in §15 is TREC21 *develop*. The recipe was frozen (λ_KL=1, KZ+`sonnet.v1` synth, LoRA r8/α8) at **step 1250 (dev 0.6072)** — chosen over dev-max step 1500 for narrative retention (canary fell 86%→80% over 1000→1500 while dev rose only +0.021, so the last steps are partly develop-specific). `eval_trec22_oneshot.ipynb` then applied that **single frozen adapter, once**, to the untouched **TREC22 NQS pool**, scoring each (topic, pool-doc) with base monoT5 + the adapter in v7's **native vdoc format** (`title:…condition:…eligibility:…`[:1400], the exact representation it was trained on — *not* the bakeoff's `truncated_doc_text`), graded NDCG@10 via `pytrec_eval`.

**Table 16.1 — TREC22 NQS pool, NDCG@10 (graded):**

| system | NDCG@10 | note |
|---|---|---|
| **v7 monoT5_CT (synth-adapted, this work)** | **0.5570** | frozen step-1250 adapter |
| zero-shot control (base, *same* vdoc + pool) | 0.4970 | adaptation added **+0.060** (paired, same 50 topics) |
| our ensemble (R/multi-view/NQS, §12) | 0.5750 | the fair same-pool bar |
| monoT5-MED zero-shot, elig_first (§13h) | 0.5220 | *different* doc format — a footnote, not the control |
| h2oloo `frocchio_monot5_e` (TREC22 winner) | 0.6125 | **full-corpus** retrieval ≠ our pool |

Three findings, read with the n=50 CI in mind (§7e: CI width ~0.13, so small cross-system gaps are not resolvable):

- **The robust result is the paired +0.060.** The clean control is base monoT5 in the *same* vdoc format on the *same* pool (adapter off via `disable_adapter()`) — its variance is within-pair, so it survives small-n far better than any cross-system comparison. **The synth-enabled adaptation transferred to the untouched test, +0.060 over zero-shot, no collapse.** (Corroborating: the vdoc control 0.4970 is *below* the §13h elig_first zero-shot 0.5220 — v7 earns its 0.557 *despite* a base doc-format that's slightly worse, so the adaptation is doing real work, not riding the representation.) Note the control isolates **adaptation**, not **synth-vs-KZ-only** (that attribution is the *develop* result, §15c) — KZ-only would almost certainly collapse on TREC22 too, but a test touch wasn't spent to show the obvious.
- **v7 (0.557) is statistically indistinguishable from our ensemble (0.575).** The honest statement is *a single adapted monoT5 matches the whole multi-view ensemble*, not "falls short." The develop→test drop (0.607→0.557) is mostly the pool-type change (TREC21 *judged* pool → TREC22 *NQS* pool, retrieval-limited), not a generalization failure.
- **It is below h2oloo's 0.6125 — not a beat.** And 0.6125 is *full-corpus*, so even clearing it on our NQS pool would be *competitive, pool-caveated*, not clean. A clean beat needs full-corpus retrieve→rerank or h2oloo's TREC22 run file for a paired test.

**Verdict.** The synthetic-data **method is a genuine, validated contribution** — the first successful `monoT5_CT` domain adaptation in seven attempts (§15), with a transferable +0.060 on the *untouched* TREC22 test. But it is **not a standalone SOTA beat**, and per the user's "no publish without a beat" bar, not yet the win. Remaining levers, in order of headroom: (1) *harder synth* to lift the develop ceiling (§17 — the difficulty gate flagged the `sonnet.v1` synth as ~90% *easier* than real TREC positives, plausibly the ceiling); (2) monoT5 as an **ensemble feature** (T5 is architecturally orthogonal to the BioLinkBERT/Qwen ensemble; §7i precedent: each independent feature added +0.005–0.019 — but reaching 0.6125 from 0.575 needs +0.038, larger than any single-feature add in the project's history, so a clean beat via the ensemble is unlikely). **Discipline:** the TREC22 one-shot is now a *spent touch*; any further eval (ensemble+v7, or a harder-synth recipe) is a pre-registered additional touch, reported regardless of outcome — no trying variants until one clears the bar.

## 17. The hardness program — the synth was too easy, and fixing it (2026-08)

§16's develop→test gap raised the question the difficulty gate (§14b) was built to answer: *is the synthetic data as hard for the base model as real TREC data, or systematically easier?* The answer was decisive and it reframes the ceiling.

**17a. The diagnosis: fidelity-high but too easy.** Scoring the verified `sonnet.v1` positives with base monoT5-MED and overlaying real TREC21 judged pairs: **90% of synth positives were easier than the median real positive** (synth mean margin +1.00 vs real −0.53). The mechanism: the generator was *handed the trial's eligibility criteria and wrote them into the note*, so base monoT5 found trivial lexical overlap — whereas a real TREC patient describes the same clinical reality in *their own* vocabulary, forcing inference. High blind-verify fidelity coexisting with low base-model difficulty is exactly the *weak-gradient / template-overfit / no-transfer* failure mode, and it is invisible to the fidelity check.

**17b. The lever is the *prompt* and the *model*, and the direction is counter-intuitive.** `v2_strict` (demand a *specific datum* per criterion) is the **wrong** lever — more explicit evidence is *easier*, not harder. The right lever is `v3_implicit`: **ban criterion-echoing; encode each target through clinical facts a reader must *infer*** ("works full-time as a roofer, hikes weekends" for a performance-status inclusion, never "ECOG 1"). Difficulty by generator, at full scale:

| generator + prompt | % easier than real median | mean margin (real = −0.53) | curated pairs |
|---|---|---|---|
| `sonnet.v1` | 90% | +1.00 | 3,942 |
| `sonnet.v3_implicit` | 85% | +0.67 | 1,673 |
| **`opus.v3_implicit`** | **67%** | **−0.09** (≈ real) | **3,558** |

`v3_implicit` on Sonnet moved the needle only modestly (90→85; the n=32 pilot's optimistic 75% didn't hold at scale). **The model was the bigger lever:** Opus executes "imply, don't state" far better, reaching **67% — nearly matched-real hardness**, its margin distribution essentially overlaying real TREC positives.

**17c. Does harder data actually *help*? — the free equal-N probe (yes).** The difficulty gate is a *proxy*; only v7 develop decides. An equal-N control (both 1,673 pairs, KL=1, 1500 steps) isolates hardness from quantity:

| step | v1-1673 (90% easy) | sonnet-v3-1673 (85%) |
|---|---|---|
| 250 | 0.4764 | 0.4642 |
| 1250 | 0.6019 | 0.6128 |
| 1500 | **0.6163** | **0.6331** |

**Harder data wins at equal quantity, +0.017**, with a *repeatable* mechanistic signature: v3 trains **slower early** (lower at 250–1000) and reaches a **higher ceiling** (crosses ~step 1250, higher at 1500) — the same crossover appeared in *both* the v1-full and v1-equal-N comparisons, so it is signal, not a single-run fluke. (sonnet-v3 at 1,673 also *beat v1's full 3,942-pair peak* 0.6132 — better data at <half the count.) The effect is small for a 5-point hardness move; Opus is a **23-point** move (90→67), motivating the full Opus run.

**17d. The full Opus dataset (the SOTA push).** `opus.v3_implicit` full run: **3,558 curated pairs, 67% easier** (hardest and full-size), with two deliberate fixes. (i) `max_tokens 1500→2500`: Opus's longer implicit notes were truncating mid-JSON ~8% of the time, biased against the *longest/hardest* notes; the bump recovered them (yield 45%→49%). (ii) **`keep_hard=True`**: the `base_disagrees` bucket (base flatly rejects a verifier-passed positive) was designed for v1 as a *fidelity-failure* drop — but for genuinely-hard synth those are the **valuable hard positives** (base can't read the implicit evidence), so they are now *kept in training* (708 pairs, 20%) instead of QA-dropped. Only 5 pairs are trivially easy (vs v1's 122).

**17e. The Opus develop result — best ever, and it never forgets.** v7 on the Opus set (KL=1, STEPS=2500):

**Table 17.1 — TREC21 dev NDCG@10 and canary retention by step (opus.v3_implicit, 3,558 pairs):**

| step | dev NDCG@10 | canary (% of base) |
|---|---|---|
| 250 | 0.4758 | — |
| 1000 | 0.5998 | 101% |
| 1500 | 0.6312 | 96% |
...
**Develop peaked at 0.6617** — beating sonnet-v3's 0.6331 by +0.029 and v1's 0.6132 by +0.049, exactly as the hardness→develop relationship (§17c) predicts, and the closest the project has come to h2oloo's `monoT5_CT` 0.7118 (gap 0.05). Dev is plateauing (last 250 steps +0.0016). **The decisive new finding is the canary: it stayed 95–103% the entire run (99% at step 2500) and never declined.** v1 and sonnet-v3 both *decayed to ~80%* by their late steps — which is why v1 was frozen *before* dev-max (§16). The Opus data reaches a higher develop ceiling **while fully retaining narrative competence** — so the "late-steps-overfit-transfer-worse" caution does not apply, and step 2500 is frozen without a tradeoff (it is simultaneously the dev-max *and* the best late-step retention). This is also a **positive transfer signal**: v1 reached 0.607 develop having decayed to 80% canary and transferred to 0.557 (−0.05); Opus reaches 0.6617 at 99% canary, so the develop→TREC22 drop should be *smaller* — plausibly landing ~0.62–0.63, at or above h2oloo's 0.6125. **Status: the recipe cleared the pre-registered bar (0.6617 ≫ sonnet-v3's 0.6331), so it earns the second TREC22 touch;** `eval_trec22_oneshot.ipynb` is pointed at `monot5_ct_v7_step2500` (⚠ the Opus run reused the `_step*` paths, so those checkpoints are Opus's now), reported alongside the v1 touch-#1 0.557 whatever it returns. *(This section gets the TREC22 number when the one-shot completes.)*

**Honest residuals.** (a) develop→TREC22 transfer is the unproven hop — v1 dropped 0.613→0.557 on the pool-type change, so even a 0.66–0.68 Opus develop may land ~0.58–0.62 on TREC22 (competitive-to-marginal-beat of 0.6125, not guaranteed clean). (b) The synth is still an *approximation* seeded from TREC21 style; a generic-narrative control for the residual quantity/style confound remains unrun. (c) Every problem in this program lived in the *scoring/filtering* layer (taxonomy-collapse, two failed relevance-gate designs, canary-selection artifact, curation dropping hard positives) — the generation was consistently sound; the standing lesson is to prefer letting the downstream number decide over adding another filter.
